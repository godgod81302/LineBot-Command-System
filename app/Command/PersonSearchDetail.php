...
class PersonSearchDetail extends BaseCommand{
	
	private static $instance;
	
	public static function getInstance(){
		if( !self::$instance ){
			self::$instance = new PersonSearchDetail();
		}
	...
	protected function process( $args=null ){
		...
		$command = mb_substr($args->command,1,mb_strlen($args->command)-1);
		...
		if( $name=='誰' ){
			...
			if ( count($servers_id)==1 ){
				...
				$temps = [];
				$temps = CommandUtil::getServerAndServicePointPhoto($server);
			...
			foreach($servers as $server){
				...
				if ( !$single_server ){
					...
					if (count(array_unique($result->periods))!=count(array_unique($server->services->where('description','<>','特殊服務')->where('status','<>','hide')->pluck('period')->all()))){
						unset($result);
						$result = CommandUtil::getServerNextFreeTime1($server,$search_datetime);
